...
const evalToString = (
  type: ts.BinaryExpression | ts.StringLiteral | ts.NumericLiteral,
): string | number | void => {
  ...
  switch (type.kind) {
    ...
    case ts.SyntaxKind.NumericLiteral:
      return parseInt(type.text, 10);
    case ts.SyntaxKind.BinaryExpression:
      switch (type.operatorToken.kind) {
        case ts.SyntaxKind.PlusToken:
          return evalToString(type.left) + evalToString(type.right);
        ...
        case ts.SyntaxKind.GreaterThanGreaterThanGreaterThanToken:
          return evalToString(type.left) >>> evalToString(type.right);
        default:
          console.error("Unsupported operator", type.operatorToken);
          return;
      }
    case ts.SyntaxKind.ParenthesizedExpression:
      return evalToString(type.expression);
    default:
      console.error("Unsupported type", kind);
      return;
  }
};

export const printExpressionAsType = (rawType: any): string => {
  const type: PrintNode = rawType;
  const kind = ts.SyntaxKind[type.kind].toString();
  switch (type.kind) {
    case ts.SyntaxKind.BinaryExpression: {
      const value = evalToString(type);
